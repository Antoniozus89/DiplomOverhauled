...
class Comment(models.Model):  # Класс Comment наследуется от models.Model, что делает его моделью Django.
    ...
    content = models.TextField()  # Это текстовое поле для хранения содержимого комментария.
    # Оно может содержать длинные тексты.
    ...
    def __str__(self):
        return f'Comment by {self.author.username} on {self.post.title}'  # Этот метод возвращает
